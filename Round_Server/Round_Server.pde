...
//    8 = Tags shortened to reduce lag [x], Round locations and velocities [x], Improve shooting protocols [x], Promote teaming [x], Improve enemies [x], Change scoring dynamics and upgrades [x], Worsen spider package (dagger,speed) [x], Termite combat package [x], Autocomplete username [x], Allow complex movement [x], Various bug fixes [x]
...
void draw() {  
  ...
  thread("respond");                          //Read and respond to client messages (can be for newHD, loadHD, clientHD, spawnHD, deleteHD, and messageHD.)
